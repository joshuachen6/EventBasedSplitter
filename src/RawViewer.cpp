...
// OpenGL function declarations will typically come from the system OpenGL headers,
// which are often included by <GLFW/glfw3.h>.
// The ImGui_ImplOpenGL3_Init() call in main.cpp loads the actual function pointers.
#include <GLFW/glfw3.h> // Make sure this provides GL types like GLuint and function declarations

RawViewer::RawViewer(std::shared_ptr<ICameraStream> stream, const std::string &title)
    : cameraStream(stream), windowTitle(title), showWindow(true), openglTextureId(0), textureWidth(0), textureHeight(0) {
  if (!cameraStream) {
    spdlog::error("RawViewer: Initialized with a null ICameraStream pointer for title '{}'.", windowTitle);
    return;
  }
...
void RawViewer::cleanupTexture() {
  if (openglTextureId != 0) {
    ...
    spdlog::debug("RawViewer '{}': OpenGL texture {} deleted.", windowTitle, openglTextureId);
    openglTextureId = 0;
    textureWidth = 0;
    textureHeight = 0;
  }
...
bool RawViewer::initializeTexture(uint32_t width, uint32_t height) {
  if (width == 0 || height == 0) {
    spdlog::error("RawViewer '{}': Attempted to initialize texture with zero dimensions.", windowTitle);
    return false;
  }

  if (openglTextureId != 0 && (textureWidth != width || textureHeight != height)) {
  ...
  if (openglTextureId == 0) {
    ...
    spdlog::debug("RawViewer '{}': OpenGL texture {} initialized ({}x{}).", windowTitle, openglTextureId, width, height);
...
void RawViewer::updateTexture() {
  if (!cameraStream || !cameraStream->isRunning() || !cameraStream->isInitialized()) {
    return;
  }
  ...
  if (openglTextureId == 0 || textureWidth != currentWidth || textureHeight != currentHeight) {
    if (!initializeTexture(currentWidth, currentHeight)) {
      spdlog::error("RawViewer '{}': Failed to initialize texture for update.", windowTitle);
      return;
    }
  }
  ...
  glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, textureWidth, textureHeight, GL_BGR, GL_UNSIGNED_BYTE, buffer);
  glBindTexture(GL_TEXTURE_2D, 0);
}
...
void RawViewer::render() {
  ...
  if (!cameraStream) {
    ...
    ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f), "Error: ICameraStream is not available.");
    ImGui::End();
    return;
  }
  ...
  if (openglTextureId != 0 && textureWidth > 0 && textureHeight > 0) {
    ImGui::Image((void *)(intptr_t)openglTextureId, ImVec2((float)textureWidth, (float)textureHeight));
